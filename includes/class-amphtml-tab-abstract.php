...
if ( ! class_exists( 'AMPHTML_Tab_Abstract' ) ) {
    ...
    abstract class AMPHTML_Tab_Abstract {
        ...
        public function display_color_field( $args ) {
            extract( $args );
            if ( ! isset( $id ) ) {
                return esc_html_e( 'ID is required settings option!', 'amphtml' );
            }
            ?>
            <p>
        ...
        public function save_order() {
            $blocks_order = array();
            $section      = sanitize_text_field( $_REQUEST[ 'current_section' ] );
            if ( get_option( self::ORDER_OPT ) ) {
                $blocks_order = maybe_unserialize( get_option( self::ORDER_OPT ) );
            }
            $blocks_order[ $section ] = sanitize_text_field( $_REQUEST[ 'positions' ] );
